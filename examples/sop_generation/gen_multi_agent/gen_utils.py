...
def get_final_agents(agents,design_states):
    ...
    for agent,style in agents.items():
        final_agents[agent] = {"roles": {}, "style": style}
        for design_state in design_states:
...
def get_relations(design_states):
    n = len(design_states)
    return {
        design_states[i]["state_name"]: {
            "0": design_states[i]["state_name"],
            "1": design_states[i + 1]["state_name"]
            if i != n - 1
            else "end_state",
        }
        for i in range(n)
...
def gen_states(design_states,index):
    ...
    for design_state in design_states:
        ...
        agent_state = {
            role: gen_agent_state(role, environment_prompt, index)
            for role in roles
        }
